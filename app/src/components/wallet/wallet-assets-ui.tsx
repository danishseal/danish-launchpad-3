...
export function PageHeader({ title, description, count }: { title: string; description: string; count?: number }) {
  return <header className="rounded-[20px] border border-[#161616] bg-[#121214] p-7"><div className="flex items-end justify-between gap-4"><div><h1 className="text-3xl font-semibold tracking-[-0.03em]">{title}</h1><p className="mt-2 text-sm text-zinc-400">{description}</p></div>{count !== undefined && <span className="text-sm text-zinc-500">{count} {count === 1 ? "asset" : "assets"}</span>}</div></header>;
}

export function ConnectState({ title, description }: { title: string; description: string }) {
  return <div className="mx-auto flex min-h-[55vh] max-w-xl flex-col items-center justify-center text-center"><span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#1a1a1e] text-[#00f090]"><Coins size={26} /></span><h1 className="mt-5 text-2xl">{title}</h1><p className="mt-2 text-sm text-zinc-500">{description}</p><ConnectButton className="mt-6 h-10 rounded-lg bg-[#00f090] px-5 text-[#10250a] hover:bg-[#00f090]" /></div>;
}
...
export function EmptyState({ icon, title, description }: { icon: ReactNode; title: string; description: string }) { return <div className="flex min-h-64 flex-col items-center justify-center rounded-[16px] border border-[#161616] bg-[#0e0e10]/80 px-6 text-center"><span className="text-zinc-600">{icon}</span><h2 className="mt-3 text-base">{title}</h2><p className="mt-2 text-sm text-zinc-500">{description}</p></div>; }
