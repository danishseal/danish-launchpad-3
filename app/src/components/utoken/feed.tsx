  ...
  return (
    <div className="rounded-xl border border-[var(--hairline)] bg-[#1c1c1e] p-3">
      <textarea
        ...
        rows={2}
        disabled={busy}
        className="w-full resize-none bg-transparent text-[14px] text-zinc-100 outline-none placeholder:text-zinc-600"
      />
      <div className="mt-2 flex items-center justify-between">
        <span className="font-mono text-[11px] text-zinc-600">{text.length}/{MAX}</span>
        <button
          type="button"
          onClick={go}
  ...
  return (
    <div className="flex gap-3 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 whitespace-pre-wrap break-words text-[14px] leading-5 text-zinc-200">{post.text}</p>
      </div>
    </div>
  );
}
...
function Empty({ label }: { label: string }) {
  return <p className="py-10 text-center text-[13px] text-zinc-500">{label}</p>;
}
