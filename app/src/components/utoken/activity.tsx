...
export function ActivityFeed({ address, compact = false }: { address?: string; compact?: boolean }) {
  ...
  if (trades.isLoading && items.length === 0) {
    return <div className="py-10 text-center text-[13px] text-zinc-500">Loading activity…</div>;
  }
  if (items.length === 0) {
    return (
      <div className="py-14 text-center">
        <p className="font-display text-[14px] font-semibold text-zinc-300">No activity yet</p>
...
function TradeRow({
  ...
  return (
    <div className="flex items-center gap-3 py-3">
      ...
      <span className="shrink-0 text-[12px] text-zinc-500">{ago(new Date(trade.time).getTime())}</span>
      <a
...
function FollowRow({ event, viewer }: { event: FollowEvent; viewer?: string }) {
  ...
  return (
    <div className="flex items-center gap-3 py-3">
      ...
      <span className="shrink-0 text-[12px] text-zinc-500">{ago(event.createdAt)}</span>
  ...
  return (
    <div className="h-9 w-9 shrink-0 overflow-hidden rounded-full bg-[#202022] ring-1 ring-inset ring-white/10">
      {image ? (
        ...
        <span className="flex h-full w-full items-center justify-center text-[12px] text-zinc-500">{symbol.slice(0, 1)}</span>
      )}
    </div>
  );
}
