...
function Empty({ hint }: { hint: string }) {
  return (
    <div className="flex h-[220px] flex-col items-center justify-center gap-1 text-center">
      <p className="font-mono text-[12px] text-zinc-500">No activity in this window</p>
      <p className="max-w-[220px] text-[11px] leading-4 text-zinc-600">{hint}</p>
    </div>
  );
}
...
export function VolumeChart({
  ...
  return (
    <div className="relative">
      ...
      <div className="pointer-events-none absolute left-1 top-1 font-mono text-[10px] text-zinc-600">
        {fmtValue(max)}
      </div>
      {/* time ticks */}
      ...
      {h && hover != null && (
        ...
        >
          ...
          <p className="mt-0.5 font-mono text-[13px] font-semibold text-white tabular-nums">
...
export function LaunchesChart({
  ...
  return (
    <div className="relative">
      ...
      <div className="pointer-events-none absolute left-1 top-8 font-mono text-[10px] text-zinc-600">
        {max}
      </div>
      <div className="mt-1 flex justify-between font-mono text-[10px] text-zinc-600">
        <span>{fmtTime(points[0].t)}</span>
      ...
      {h && hover != null && (
        ...
        >
          <p className="font-mono text-[10px] text-zinc-500">{fmtTime(h.t)}</p>
          <p className="mt-0.5 flex items-center gap-1.5 font-mono text-[11px] text-zinc-200 tabular-nums">
            <span className="inline-block h-2 w-2 rounded-[2px]" style={{ background: LAUNCHED }} />
            {h.launched} launched
          </p>
          <p className="flex items-center gap-1.5 font-mono text-[11px] text-zinc-200 tabular-nums">
...
function Legend() {
  return (
    <div className="mb-2 flex items-center gap-4 font-mono text-[11px] text-zinc-400">
