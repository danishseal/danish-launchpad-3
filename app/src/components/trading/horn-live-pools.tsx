  ...
  return (
    <section className="rounded-2xl border border-[#1e1e22] bg-[#0e0e10]/80 p-5">
          ...
          {history.length > 0 ? (
            <button
              ...
              className={`rounded-[4px] border px-2 py-0.5 font-mono text-[10px] transition-colors ${
                showHistory
                  ? "border-[#3a3a42] bg-[#1c1c1e] text-zinc-200"
      ...
      {showHistory && history.length > 0 ? (
        <div className="mt-4 border-t border-[#1e1e22] pt-4">
            ...
            {history.map((r) => (
              <li key={r.token.address}>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-3 rounded-lg px-2.5 py-2 transition-colors hover:bg-[#131316]"
                >
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[13px] font-semibold text-zinc-300">
                      ${r.token.symbol}{" "}
                      <span className="font-normal text-zinc-600">{r.horn.name ?? r.horn.slug}</span>
                    </span>
                  ...
                  <ArrowSquareOut size={13} className="shrink-0 text-zinc-600" />
                </a>
              </li>
            ))}
          </ul>
