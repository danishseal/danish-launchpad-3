      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <label className="relative block w-full sm:w-72">
          ...
          <Input
            ...
            autoComplete="off"
            className="h-9 rounded-lg border-[#040d09] bg-[#020604] pl-9 text-xs text-zinc-200 placeholder:font-semibold placeholder:text-zinc-600"
          />
        </label>
      </div>
...
function Tab({ active, onClick, icon, children }: { active: boolean; onClick: () => void; icon: React.ReactNode; children: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex h-10 items-center gap-2 rounded-[8px] px-4 text-sm transition-colors ${active ? "bg-[#040d09] text-zinc-100" : "text-zinc-300 hover:bg-[#161619]"}`}
    >
      {icon}
      {children}
    </button>
