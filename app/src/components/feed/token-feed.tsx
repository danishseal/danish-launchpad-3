  ...
  return (
    <div className="mx-auto max-w-[1440px] space-y-8">
        ...
        <div className="-mx-1 flex gap-2.5 overflow-x-auto px-1 pb-1">
          {isLoading
            ? Array.from({ length: 8 }).map((_, i) => (
                <div
                  key={i}
                  className="h-[236px] w-[176px] shrink-0 animate-pulse rounded-[8px] border border-[#1a1a1e] bg-[#0e0e10]/80"
                />
              ))
      ...
      <section className="space-y-3">
        ...
        />
        <div className="grid grid-cols-2 gap-px overflow-hidden rounded-[8px] border border-[#1a1a1e] bg-[#1a1a1e] lg:grid-cols-4">
          <ProtocolStat
      ...
      <section className="space-y-3">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link
            href="/explore"
            className="flex items-center gap-1.5 font-display text-[11px] uppercase tracking-[0.14em] text-zinc-500 transition-colors hover:text-zinc-200"
          >
            browse all <ArrowRight size={12} />
          </Link>
        </div>
        {error && (
          <div className="rounded-[8px] border border-[#3a1418] bg-[#1a0c0e] p-4 text-center text-sm text-[#ff5b5b]">
            Failed to load coins.
          </div>
        )}
        <div className="overflow-x-auto rounded-[8px] border border-[#1a1a1e] bg-[#0c0c0e]/80">
          <div className="min-w-[720px]">
            <div className="grid grid-cols-[minmax(240px,1fr)_140px_120px_110px_90px] items-center border-b border-[#1a1a1e] px-4 py-2.5">
              {["Coin", "Market Cap", "Progress", "Age", "Activity"].map((h, i) => (
                <span
                  key={h}
                  className={`font-display text-[11px] font-medium uppercase tracking-[0.14em] text-zinc-600 ${i > 0 ? "text-right" : ""}`}
                >
                  {h}
                </span>
              ))}
            ...
            {isLoading
              ? Array.from({ length: 8 }).map((_, i) => (
                  <div
                    key={i}
                    className="grid grid-cols-[minmax(240px,1fr)_140px_120px_110px_90px] items-center border-b border-[#141417] px-4 py-2.5 last:border-b-0"
                  >
                    <div className="flex items-center gap-2.5">
                      <div className="h-9 w-9 shrink-0 animate-pulse rounded-[5px] bg-[#141417]" />
                      <div className="h-3 w-28 animate-pulse rounded bg-[#141417]" />
                    </div>
                  </div>
                ))
...
function HornsPromoBanner() {
  ...
  return (
    <Link
      href="/horns"
      className="group relative block w-full overflow-hidden rounded-[10px] border border-[#1f2a20] transition-colors hover:border-[#2c4030]"
...
function LaunchFooterBar() {
  return (
    <section className="rounded-[10px] border border-[#1a1a1e] bg-[#0c0c0e]/80 px-5 py-4">
...
function FeeShareBar({
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="w-16 shrink-0 font-mono text-[11px] text-zinc-400">{label}</span>
      <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-[#1a1a1e]">
        <span className="block h-full rounded-full" style={{ width: `${value}%`, background: tone }} />
      </div>
      <span className="mono w-10 shrink-0 text-right text-[11px] font-semibold text-zinc-300">
        {display}
      </span>
    </div>
  );
...
function RankedCoin({ token, rank }: { token: TokenListItem; rank: number }) {
  ...
  return (
    <Link
      ...
      className="group relative flex w-[176px] shrink-0 flex-col overflow-hidden rounded-[8px] border border-[#1a1a1e] bg-[#0e0e10]/80 transition-colors hover:border-[#2a2a30]"
    >
      ...
      <div className="flex flex-col gap-1 p-2.5">
        ...
        <p className="truncate font-mono text-[11px] text-zinc-500">${token.symbol ?? "TOKEN"}</p>
...
function ProtocolStat({
  ...
  return (
    <div className="bg-[#0a0a0b] px-4 py-4">
      ...
      <p className="mt-2.5 text-[11px] text-zinc-600">{foot}</p>
    </div>
  );
}
    ...
    >
      <div className="flex min-w-0 items-center gap-2.5">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-1.5">
            <p className="truncate text-[13px] font-semibold text-zinc-100">{token.name ?? "Unknown"}</p>
            <span className="shrink-0 font-mono text-[10px] uppercase text-zinc-600">[{token.symbol ?? "TKN"}]</span>
          </div>
          <button
            type="button"
            onClick={(e) => {
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <span className="mono w-7 text-right text-[11px] text-zinc-500">
          {token.graduated ? "AMM" : progress == null ? "·" : `${progress.toFixed(0)}%`}
        </span>
      </div>
      <span className="text-right text-[12px] text-zinc-500">{age}</span>
      <span className="mono text-right text-[12px] text-zinc-400">{activity} tx</span>
    </Link>
  );
}
