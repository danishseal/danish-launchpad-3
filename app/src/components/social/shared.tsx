  ...
  return (
    <div className="min-w-0 leading-tight">
      <div className="flex items-center gap-x-1.5">
        ...
        <span
          className="shrink-0 font-mono text-[12px] text-zinc-600"
  ...
  return (
    ...
    >
      {src ? (
        ...
        <User size={iconSize} weight="fill" className="text-zinc-600" />
      )}
    </span>
  );
}
    ...
    <div className="flex items-center gap-2.5 rounded-xl border border-[var(--hairline)] bg-[#161616] px-3 py-2 transition-colors hover:border-[var(--hairline-strong)]">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          {token?.name && (
            <span className="truncate font-sans text-[12px] text-zinc-500">{token.name}</span>
          )}
        </div>
        {token && (
