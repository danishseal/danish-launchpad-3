  ...
  return (
    <div className="flex max-h-[70vh] flex-col">
      <div className="flex h-11 shrink-0 items-center justify-between border-b border-[var(--hairline)] px-4">
        <span className="font-display text-[12px] font-semibold uppercase tracking-[0.14em] text-zinc-400">
          Notifications
        </span>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {!connected ? (
          <div className="flex flex-col items-center gap-2 px-6 py-8 text-center">
            <Bell size={26} weight="duotone" className="text-zinc-600" />
            <p className="text-[13px] text-zinc-400">Connect your wallet to see notifications.</p>
            <button
              type="button"
              onClick={onConnect}
          ...
          <p className="px-4 py-6 text-[13px] text-[#ff5b5b]">{errMsg(notifs.error)}</p>
        ) : items.length ? (
          <ul>
            {items.map((n) => (
          ...
          <div className="px-6 py-8 text-center">
            <p className="text-[13px] text-zinc-400">You&apos;re all caught up.</p>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-baseline gap-1.5">
            <span className="truncate text-[13px] text-zinc-200">
              <span className="font-semibold text-zinc-100">{identity.name}</span>{" "}
              <span className="text-zinc-400">{verbFor(n.kind)}</span>
            </span>
          </div>
          {n.preview && (
            <p className="mt-0.5 truncate text-[12px] text-zinc-500">{n.preview}</p>
          )}
