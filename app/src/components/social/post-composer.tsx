  ...
  if (!wallet.address) {
    return (
      <div className="flex flex-col items-center gap-3 border-b border-[var(--hairline)] px-4 py-8 text-center">
        <p className="font-sans text-[14px] text-zinc-300">
      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <textarea
            ...
            className="w-full resize-none bg-transparent text-[15px] leading-6 text-zinc-100 outline-none placeholder:text-zinc-600"
          />
          ...
          <div className="mt-2 flex items-center justify-between border-t border-[var(--hairline)] pt-2">
            <div className="flex items-center gap-1">
              ...
              />
              {imgBusy && <span className="font-mono text-[11px] text-zinc-500">Processing…</span>}
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              {onCancel && (
                <button
                  type="button"
                  onClick={onCancel}
                  className="h-8 rounded-lg px-3 font-sans text-[13px] font-medium text-zinc-400 hover:text-zinc-200"
                >
                  Cancel
                </button>
              )}
  ...
  return (
    <div className="mt-2 flex items-center gap-2.5 rounded-xl border border-[var(--hairline)] bg-[#161616] px-3 py-2">
      ...
      <div className="min-w-0 flex-1">
        ...
        {token?.name && (
          <span className="ml-1.5 truncate font-sans text-[12px] text-zinc-500">{token.name}</span>
        )}
      </div>
      <button
        type="button"
    ...
    <div className="ansem-fade-in mt-2 overflow-hidden rounded-xl border border-[var(--hairline)] bg-[#161616]">
      <div className="flex items-center gap-2 border-b border-[var(--hairline)] px-3 py-2">
        ...
        <input
          ...
          className="h-6 flex-1 bg-transparent font-sans text-[13px] text-zinc-100 outline-none placeholder:text-zinc-600"
        />
        <button
          type="button"
          onClick={onClose}
      ...
      <ul className="max-h-[240px] overflow-y-auto">
        {results.length === 0 ? (
          <li className="px-3 py-4 text-center font-sans text-[12px] text-zinc-600">No coins found.</li>
        ) : (
          results.map((t) => (
            <li key={t.address}>
              <button
...
function QuotedEmbed({ post }: { post: Post }) {
  ...
  return (
    <div className="mt-2 rounded-xl border border-[var(--hairline)] bg-[#161616] p-3">
      ...
      {post.text && (
        <p className="mt-1 line-clamp-4 whitespace-pre-wrap break-words font-sans text-[13px] leading-5 text-zinc-300">
          {post.text}
        </p>
      )}
    </div>
