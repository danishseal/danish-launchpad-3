...
function Loading({ label }: { label: string }) {
  return <p className="px-4 py-16 text-center font-sans text-[13px] text-zinc-600">{label}</p>;
}
  ...
  return (
    <button
      ...
      className={`shrink-0 rounded-full font-sans font-semibold transition-colors disabled:opacity-40 ${pad} ${
        following
          ? "border border-[var(--hairline)] bg-transparent text-zinc-300 hover:border-[#ff5b5b] hover:text-[#ff5b5b]"
    ...
    <div className="flex items-center gap-3 px-3 py-2.5">
      ...
      <Link href={`/creator/${profile.address}`} className="min-w-0 flex-1">
        <div className="flex items-center gap-1">
          <span className="truncate font-sans text-[13px] font-semibold text-zinc-100 hover:underline">
            {name}
          </span>
          {profile.verified && (
            <span
        ...
        <div className="flex items-center gap-1.5 font-mono text-[11px] text-zinc-500">
          ...
          {profile.mutual > 0 && <span className="text-zinc-600">· {profile.mutual} in your circle</span>}
        </div>
      </Link>
...
export function WhoToFollow({ limit = 12, title }: { limit?: number; title?: string }) {
  ...
  return (
    <div>
      {title && (
        <h2 className="mb-2 px-1 font-display text-[16px] font-semibold text-white">{title}</h2>
      )}
      {isLoading ? (
        ...
        <div className="divide-y divide-[var(--hairline)] overflow-hidden rounded-xl border border-[var(--hairline)] bg-[#030806]">
          {data.map((p) => (
        ...
        {(Object.keys(TRENDING_WINDOWS) as TrendingWindow[]).map((w) => (
          <button
            ...
            className={`rounded-full px-3 py-1 font-sans text-[12px] font-medium transition-colors ${
              window === w ? "bg-[#08160f] text-white" : "text-zinc-500 hover:text-zinc-300"
            }`}
          >
            {WINDOW_LABELS[w]}
          </button>
    ...
    <div className="overflow-hidden rounded-xl border border-[var(--hairline)] bg-[#030806]">
      {tags.map((t, i) => (
        <button
          ...
          className="flex w-full items-center gap-3 border-b border-[var(--hairline)] px-4 py-3 text-left transition-colors last:border-b-0 hover:bg-[#08160f]"
        >
    ...
    <div>
      <div className="mb-3 flex items-center gap-2">
        <button
          type="button"
          onClick={onBack}
          className="rounded-md px-2 py-1 font-sans text-[12px] text-zinc-500 transition-colors hover:bg-[#08160f] hover:text-zinc-300"
        >
          ← Tags
        </button>
        <h2 className="font-display text-[18px] font-semibold text-white">#{tag}</h2>
      </div>
      {isLoading ? (
...
export function TrendingTagsStrip() {
  ...
  return (
    <div className="mb-4 flex items-center gap-2 overflow-x-auto rounded-xl border border-[var(--hairline)] bg-[#030806] px-3 py-2.5">
      ...
      {list.map((t) => (
        <Link
          ...
          className="shrink-0 rounded-full border border-[var(--hairline)] px-2.5 py-1 font-mono text-[12px] text-[#00f090] transition-colors hover:bg-[#08160f]"
        >
          #{t.tag}
        </Link>
      ))}
      <Link
        ...
        className="ml-auto shrink-0 font-sans text-[12px] text-zinc-500 hover:text-zinc-300"
      >
        See all
      </Link>
    </div>
