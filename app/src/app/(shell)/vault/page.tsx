  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6 font-sans">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="shrink-0 rounded-[4px] border border-[var(--hairline)] px-2 py-0.5 font-mono text-[10px] text-zinc-500">
          {live ? "live" : "preview"}
        </span>
      </div>
      ...
      <div className="rounded-2xl border border-[var(--hairline)] bg-[#1c1c1e] p-5">
        ...
        <p className="mt-3 flex items-center gap-1.5 text-[11px] leading-4 text-zinc-600">
          <Horse size={12} weight="fill" className="text-zinc-600" />
          {live
...
function Reward({ denom, value }: { denom: string; value: string }) {
  return (
    <div className="flex items-center justify-between">
      <span className="mono text-[11px] text-zinc-500">{denom}</span>
      <span className="mono text-[13px] font-semibold text-zinc-300">{value}</span>
    </div>
  );
}
