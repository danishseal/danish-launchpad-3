      ...
      <Card title="Top tokens by volume" subtitle={labelFor(win)} noPad>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[560px] border-collapse">
            <thead>
              <tr className="border-b border-[var(--hairline)] text-left font-mono text-[10px] uppercase tracking-[0.1em] text-zinc-500">
            ...
            <tbody>
                ...
                Array.from({ length: 6 }).map((_, i) => (
                  <tr key={i} className="border-b border-[var(--hairline)]">
                    <td colSpan={5} className="px-4 py-3">
                      <div className="h-4 w-full animate-pulse rounded bg-[#232325]" />
                    </td>
                  </tr>
                ))}
              {!top.isLoading && (top.data?.tokens.length ?? 0) === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-center text-[13px] text-zinc-500">
              ...
              {top.data?.tokens.map((t, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-2.5 text-right font-mono text-[12px] text-zinc-200 tabular-nums">
                    {t.volumeUsd != null
                      ? usdCompact(t.volumeUsd)
                      : `${num(Math.round(t.volumeBase))} ${t.base_label}`}
                  </td>
                  <td className="px-4 py-2.5 text-right font-mono text-[12px] text-zinc-300 tabular-nums">
...
function StatTile({
  ...
  return (
    <div className="bg-[#000000] p-4">
      ...
      {loading ? (
        <div className="mt-2 h-6 w-20 animate-pulse rounded bg-[#232325]" />
      ) : (
        <p
    ...
    <div className="rounded-2xl border border-[var(--hairline)] bg-[#030806]">
      <div className="flex items-start justify-between gap-4 border-b border-[var(--hairline)] px-4 py-3">
        <div>
          ...
          {subtitle && <p className="mt-0.5 font-mono text-[10px] uppercase tracking-[0.08em] text-zinc-500">{subtitle}</p>}
        </div>
        {right}
      </div>
