  ...
  return (
    <div className="font-sans text-zinc-100">
      ...
      <div className="max-w-3xl">
        ...
        <h1 className="mt-3 font-display text-[32px] font-bold leading-[1.05] tracking-tight text-white">
        ...
        <p className="mt-3 text-[14px] leading-6 text-zinc-400">
          ...
          <span className="text-zinc-200">Horn Vault</span>, where ANSEM and CHANSE stakers earn it, or
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="space-y-4">
            {HORN_CATEGORIES.map((cat) => {
              ...
              return (
                <div key={cat}>
                  <p className="px-1 pb-1.5 font-mono text-[10px] uppercase tracking-[0.14em] text-zinc-600">
                    {cat}
                  </p>
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="min-w-0 flex-1">
                            ...
                            <span className="block truncate text-[11px] text-zinc-600">{h.tagline}</span>
                          </span>
                        </button>
                      );
                    })}
      ...
      <section className="mt-8 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-[#1e1e22] bg-[#0e0e10]/80 p-5">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-[13px] leading-6 text-zinc-400">
            ...
            <span className="font-mono text-zinc-300">before_swap</span> to price or gate a trade,{" "}
            <span className="font-mono text-zinc-300">after_swap</span> to act on it. The Vault takes
        ...
        <Link
          href="/create"
          className="inline-flex shrink-0 items-center gap-1.5 rounded-[4px] border border-[#26262b] bg-[#101012] px-3.5 py-2 font-display text-[11px] font-bold uppercase tracking-[0.14em] text-zinc-200 transition-colors hover:border-[#3a3a42] hover:text-white"
        >
  ...
  return (
    <section className="rounded-2xl border border-[#1e1e22] bg-[#0e0e10]/80 p-5">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-[4px] border border-[#26262b] bg-[#141416] px-2 py-0.5 font-mono text-[10px] font-semibold uppercase tracking-wider text-zinc-500">
              {horn.category}
            </span>
          </div>
          <p className="mt-1 text-[13px] text-zinc-500">{horn.tagline}</p>
        </div>
      </div>

      <p className="mt-3 text-[14px] leading-6 text-zinc-300">{horn.blurb}</p>

      <div className="mt-4 flex flex-wrap gap-1.5">
        {horn.hooks.map((h) => (
          <span key={h} className="rounded-md border border-[#26262b] bg-[#141416] px-2 py-1 font-mono text-[11px] text-zinc-400">
            {h}
          </span>
        ))}
      </div>
      ...
      <div className="mt-5 rounded-xl border border-[#1e1e22] bg-[#0a0a0b] p-4">
        ...
        <p className="mt-2 text-[13px] leading-6 text-zinc-400">{horn.example}</p>
      </div>
    </section>
  );
}
